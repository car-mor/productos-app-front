<template>
  ...
  <div v-for="notification in notifications" :key="notification.id" class="toast show align-items-center text-bg-success border-0" role="alert" aria-live="assertive" aria-atomic="true">
    <div class="d-flex">
      <div class="toast-body">
        {{ notification.message }}
      </div>
      <button type="button" class="btn-close btn-close-white me-2 m-auto" @click="closeNotification(notification.id)"></button>
    </div>
  </div>
</div>

      <!-- Botón para agregar un nuevo producto -->
      <button class="btn btn-primary me-4 mb-4 px-4" data-bs-toggle="modal" data-bs-target="#productModal" @click="openAddProductModal">
        <i class="bi bi-plus-circle me-2"></i>Agregar Producto
      </button>

      <!-- Botón para agregar nueva categoría -->
      <button class="btn btn-primary me-4 mb-4 px-4" data-bs-toggle="modal" data-bs-target="#categoryModal" @click="openAddCategoryModal">
        <i class="bi bi-folder-plus me-2"></i>Agregar Categoría
      </button>

      <!-- Botón para agregar un nuevo proveedor -->
        <button class="btn btn-primary me-4 mb-4 px-4" data-bs-toggle="modal" data-bs-target="#providerModal" @click="openAddProviderModal">
            ...
            <form @submit.prevent="saveCategory">
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input v-model="currentCategory.name" type="text" class="form-control" id="categoryName" required />
                </div>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
            ...
            <form @submit.prevent="saveNewProvider">
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input v-model="newProvider.name" type="text" class="form-control" id="newProviderName" required />
                </div>
                <div class="mb-3">
                  <label for="newProviderPhone" class="form-label">Teléfono</label>
                  <input v-model="newProvider.phone" type="text" class="form-control" id="newProviderPhone" required />
                </div>
                <div class="mb-3">
                  <label for="newProviderUrl" class="form-label">URL</label>
                  <input v-model="newProvider.url" type="url" class="form-control" id="newProviderUrl" required />
                </div>
              </div>
              <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
          ...
          <div class="modal-content">
            ...
            <div class="modal-body">
              ...
              <div class="mb-3">
                ...
                <p><a :href="selectedProvider.url" target="_blank">{{ selectedProvider.url }}</a></p>
              </div>
              <div class="mb-3">
                <label class="form-label fw-bold">Estado:</label>
                <p>
                  <span class="badge" :class="selectedProvider.active ? 'bg-success' : 'bg-danger'">
